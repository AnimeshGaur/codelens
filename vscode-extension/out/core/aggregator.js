        ...
        if (r.architecture) {
            ...
            if (Array.isArray(arch.patterns)) {
                for (const p of arch.patterns)
                    if (p) {
                        model.architecture.patterns.add(p);
                    }
            }
            if (arch.techStack) {
                for (const key of ['languages', 'frameworks', 'databases', 'messageBrokers', 'other']) {
                    if (Array.isArray(arch.techStack[key])) {
                        for (const item of arch.techStack[key]) {
                            if (item) {
                                model.architecture.techStack[key].add(item);
                            }
                        }
                    }
                }
            ...
            if (arch.securitySurface) {
                ...
                for (const ep of sec.protectedEndpoints || []) {
                    if (ep) {
                        model.architecture.securitySurface.protectedEndpoints.add(ep);
                    }
                }
                for (const ep of sec.unprotectedEndpoints || []) {
                    if (ep) {
                        model.architecture.securitySurface.unprotectedEndpoints.add(ep);
                    }
                }
                for (const p of sec.dbWritePaths || []) {
                    if (p) {
                        model.architecture.securitySurface.dbWritePaths.add(p);
                    }
                }
            }
            if (arch.summary) {

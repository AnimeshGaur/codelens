...
exports.generateDbDiagram = generateDbDiagram;
exports.generateEndpointDiagram = generateEndpointDiagram;
exports.generateArchitectureDiagram = generateArchitectureDiagram;
exports.generateExternalApiDiagram = generateExternalApiDiagram;
exports.generateFlowDiagram = generateFlowDiagram;
// ─── Shared helpers ─────────────────────────────────────────────────────────
...
// ─── Component Diagram ──────────────────────────────────────────────────────
function generateComponentDiagram(components) {
...
function generateClassDiagram(classes) {
    ...
    for (const [file, fileClasses] of fileGroups) {
        if (useNamespaces) {
            lines.push(`    namespace ${file.replace(/[^a-zA-Z0-9_]/g, '_')} {`);
        }
        for (const cls of fileClasses) {
            ...
            if (cls.type === 'interface') {
                ...
                lines.push(`${indent}    <<abstract>>`);
            }
            else if (cls.type === 'enum') {
                lines.push(`${indent}    <<enumeration>>`);
            }
    ...
    for (const cls of classes) {
        ...
        for (const iface of cls.implements || []) {
            if (!validNames.has(safeName(iface))) {
                continue;
            }
...
// ─── Database (ER) Diagram ──────────────────────────────────────────────────
function generateDbDiagram(database) {
...
// ─── Endpoint Diagram ────────────────────────────────────────────────────────
function generateEndpointDiagram(endpoints) {
    ...
    for (const ep of endpoints) {
        const resource = (ep.path || '/root').replace(/^\//, '').split('/').filter((s) => !/^(v\d+|api)$/i.test(s) && !s.startsWith(':'))[0] || 'root';
        if (!groups.has(resource)) {
            groups.set(resource, []);
        }
        groups.get(resource).push(ep);
    }
    const methodColor = { GET: '#61affe', POST: '#49cc90', PUT: '#fca130', PATCH: '#50e3c2', DELETE: '#f93e3e' };
...
function generateArchitectureDiagram(architecture, fullModel = {}) {
    const lines = ['graph TD'];
    ...
    if (controllers.length > 0 || (fullModel.endpoints || []).length > 0) {
        ...
        controllers.slice(0, 6).forEach((c, i) => {
    ...
    if (models.length > 0 || (fullModel.database?.models || []).length > 0) {
        ...
        lines.push('        direction LR');
        (fullModel.database?.models || models).slice(0, 6).forEach((m, i) => {
...
function generateExternalApiDiagram(externalAPIs) {
...
// ─── Flow Diagram ────────────────────────────────────────────────────────────
function generateFlowDiagram(flows) {
